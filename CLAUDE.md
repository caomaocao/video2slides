...
1. **Fetch layer** (`scripts/fetch.py`, deterministic) — `yt-dlp` pulls a low-res proxy stream (~360p) used for *all* analysis, plus subtitles/chapters/heatmap/danmaku/metadata. ASR is a pluggable fallback (`ASR_BACKEND=groq|funasr|none`); `--transcript <path>` can bypass subtitle/ASR entirely.
