...
This repo is the codebase for **Cuepoint**: a Claude Code skill that turns a video (YouTube / Bilibili / local file) into a self-contained **video index document** (`video_index.json` + `frames/` — the outline↔transcript↔frames mapping as a versioned public contract, spec v0.5), rendered downstream into a `frontend-slides`-style HTML deck and/or markdown notes, with keyframes/clips that jump back to the source video's timestamp.

The full design is written up in `docs/cuepoint-spec-v0.5.md` (Chinese; v0.5 是 2026-07-13 拷问式 review 定稿的「制品升格」决议版 — v0.4/v0.3 kept as historical snapshots). **Read it before making architectural decisions** — it is the source of truth; the summary below exists so you don't have to re-read all ~450 lines just to get oriented. Implementation status: slice 1 (subtitled slide-driven online video → outline → frames → HTML end-to-end), slice 2 (ASR three families + local file input), slice 3 (long-video per-chapter outlines), **slice 4 (index-document 升格: `storyboard.py export` + `schemas/video_index.schema.json` + dedup 标注化 + 交互点形态扩展 + `scripts/notes.py` 笔记渲染器)**, and the **cross-platform precheck** batch (Windows hard-block / distro-aware install hints / `$XDG_CONFIG_HOME` / arm64-only-backend arch guard) are all implemented, review-closed and merged to main (as of 2026-07-13, 180 tests). Slice 4's four acceptance runs (slide-driven #13 / screen-recording #11p02 / talking-head local#9 / chaptered long-video #12) all passed QA — see `docs/test-videos.md` (27 verified test sources + all production findings). Next in the backlog (all P2, none started): **cross-node dedup criteria upgrade** (coding-screen-recording is now the top false-positive scenario — #12 saw 41/43 frames falsely merged; the annotation model already makes this a pure signature swap, no contract change), **notes.py scaffolding i18n** (spec §9.5 — hardcoded-Chinese framework text mixes with non-Chinese output), and a **local-file subtitle-reuse** patch (sidecar `.srt`/`.vtt` + embedded track before falling to ASR).
...
5. **Export + render layer** (host Claude + renderer family). Cross-node dedup (`storyboard.py dedup`) is **annotation-based** — it tags duplicate frames with `dedup_group`/`dedup_primary` rather than deleting them; the host arbitrates false merges by editing tags (split group / swap primary), never by restoring deleted data. Analysis then closes with `storyboard.py export` (SKILL step 5.5) which **unconditionally** packages the read-only `video_index.json` + `frames/` contract. The single interaction point asks form (slides / notes / both / index-doc-only) + Length + language. Renderers: **markdown notes** (`scripts/notes.py`, deterministic, reads the exported doc only) and/or **slides** — for slides, shallow outline nodes aggregate top-k media from their leaf descendants by pure sorting over `storyboard.json` (no re-analysis to change granularity), the slide view enforces "one frame per page" over the dedup tags, `scripts/frames.py --finalize` lazily re-fetches high-res frames/clips *only* for media that makes the page, and `frontend-slides` renders the fixed 1920×1080 HTML deck; timestamp badges deep-link to `youtube.com/watch?v=…&t=…` / `bilibili.com/video/BV…?p=n&t=…`, or (local files) open an embedded `<video src="file://…">` overlay.
...
- **`video_index.json` + `frames/`** — the exported public contract (spec §6.5): single JSON embedding full transcript segments (`timestamp_granularity` made explicit) + outline tree + proxy-quality selected frames, relative paths, validated against `schemas/video_index.schema.json`, `schema_version` field. All renderers' semantic input comes from this document only; it's read-only after export. Produced unconditionally by `storyboard.py export` at analysis close (SKILL step 5.5), before the single interaction point. Runtime validation is a hand-written stdlib subset; `jsonschema` is a **dev-only** test dependency that cross-checks exports against the schema.
...
```
cuepoint/
├── SKILL.md            # orchestration contract: precheck → fetch → signals → analyze → select frames → dedup-annotate → export → render (slides/notes)
├── scripts/
...
**Supported platforms**: macOS (Apple Silicon / Intel) and Linux/Unix (arm64 / x86_64 — Ubuntu/Debian, CentOS/RHEL/Fedora). **Native Windows is hard-blocked** by `setup.py` (exit 1; WSL2 reports as Linux and runs fine) — the scripts assume POSIX (`~/.config`, funasr venv's `bin/python`, path separators). The config dir honors `$XDG_CONFIG_HOME` (falls back to `~/.config/video2slides/`). Linux branch logic is unit-tested via monkeypatch but not yet run on real CentOS/Ubuntu/RHEL hardware.
