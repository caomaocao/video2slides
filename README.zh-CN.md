...
**把任意视频转成自包含、可回放的幻灯片——每一帧、每一句都深链回源视频最相关的那一刻。**

一个可移植的**智能体技能(agent skill）**（Claude Code 及其他智能体运行时），它*替你看完整段视频*:转写、带证据引用地列大纲、挑选关键帧,再渲染成可一键跳回视频的幻灯片或可导航笔记。

**幻灯片只是其中一个视图。Cuepoint 真正的产出,是一份机器可读的 `大纲 ↔ 逐字稿 ↔ 帧` 映射——一层「视频理解」中间产物,别的 Agent / 流水线拿去就能读懂这段视频,而无需真的看完。**
...
它不是字幕工具,也不是套在 YouTube 上的摘要器。它构建的是一份**视频索引文档**——`大纲 ↔ 逐字稿 ↔ 帧` 的持久、自包含、**机器可读**映射,别的 Agent / 流水线拿去就能对这段视频做推理而无需解码原片;幻灯片与笔记只是它面向人的两个*视图*。
...
**这个——而不是幻灯片——才是 Cuepoint 真正有意思的地方。** `video_index.json` 是一层紧凑、自包含的**「视频理解」中间产物**:讲了什么(全量逐字稿)、哪些要紧(每个节点都带原文引用的层级大纲)、长什么样(选出的关键帧),全部按时间戳交叉链接。把它丢给另一个 Agent、一个 RAG 索引、一条检索流水线,或你自己的工具,它就能回答*「这段视频讲了什么、在第几秒、把那一帧给我看」*——**无需触碰原始媒体,也无需重跑视觉模型**。幻灯片与笔记只是它的头两个消费者。
