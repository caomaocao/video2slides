...
**平台分区去锚先验【2026-07-14 增补】**:B 站分区 / YouTube `categories`+`tags`(fetch 已落 `ytdlp_info.json`,fail-open 可缺)可作轴 A 的**不对称**先验——**只去锚、不定性**。**实测【2026-07-14, yt-dlp 2026.07,样本 3 YT + 4 B 站】**:YouTube `categories` 稳定给**单个干净值**(`Education` / `Science & Technology` / `News & Politics`,3/3),可直接当体裁软提示;而 **B 站官方分区(tname)yt-dlp 一律不透出,`categories` 恒空(4/4)**,唯一沾边的是 `tags`——UP 主/活动/推广标签与话题词混排、有噪声且 ≠ 官方分区,实例:①汽车区 UP 一支「大海中央酒店探秘」vlog,`tags` 仍是 `['阿B车友俱乐部']`;②一条足球分析视频,`tags` 首几位被 `淘宝闪购夏日拼团季`/`淘宝闪购猜球大使…` 广告标签占满,真话题词(C罗/英格兰)被埋。故 B 站侧尤其只能软用、必被 transcript 语义覆盖,硬表会被带沟里——再次反证 (b-软)+去锚不建表的决定。这恰好反证 (b-软)+去锚不建表的决定:软提示可被 transcript 语义覆盖,硬表会被误导。演出/虚构/二创类分区(番剧/国创/动画/电影/电视剧/影视/鬼畜/音乐/舞蹈;YouTube `Film & Animation`/`Music`)是强「非 slide-driven / 可能非目标」信号,用来把判断**推离** slide-driven;但**永远不得单凭分区把视频定成 slide-driven**,定性仍由探针 contact sheet 目验独占(与 §12「数值判据不可独立成立」同规格)。此不对称只保留纠错方向——批量试产的假 slide-driven(#3 cinematic / #6 录屏 / #18 画板)全落在「误判为 slide」这一侧——不开「分区→锚向 slide」的反向口子。判为演出/虚构非目标时如实告知用户会降级(帧退化为氛围/装饰),但遵守零打断契约,提醒并入渲染前交互点(§9)。
...
**判定方式**:宿主 Claude 基于 transcript + 标题/简介/chapters + 平台分区(B 站分区 / YouTube `categories`+`tags`,`ytdlp_info.json` 内,fail-open 可缺)。

体裁集合:课程/教程、演讲/分享、访谈/播客、评测/对比、资讯/解读、会议记录、vlog/生活、直播实录/秀场、纪录片(共 9 类,与 SKILL.md 步骤 3/8 对齐)。每体裁对应一套大纲骨架与 frontend-slides 风格倾向(v0.4 起风格由此表**直接定**而非仅"倾向",见 §9)。

**平台分区是软提示,非硬映射【2026-07-14 增补】**:平台分区是「主题域」(视频讲什么:美食/汽车/游戏/知识),体裁是「生产形态」(哪种片:课程/访谈/评测),二者**正交**——一个分区可对多种形态(美食可以是教程/vlog/评测,科技数码可以是课程/评测/资讯),故**不建「分区→体裁」对照表**,分区只作与 transcript/标题同权的一个额外判定输入,最终由语义定。这与两轴正交、避免枚举组合矩阵的原则一致。
...
**P2 候选**:visual_form 分段分类(score 曲线粗分段);whiteboard 延时 clip;whiteboard 擦除检测(擦除大峰前取帧,替代机械末帧);嵌入原平台播放器 iframe(带 `t` 参数就地播放,本地内嵌 `<video>` 弹层已为此铺路);实拍演示独立形态;画中画区域检测;多视频 compare 模式;本地 ASR 槽位后端落地(mlx-whisper、whisper.cpp,§10.1);批量/分P 处理;marketplace 上架适配;**notes.py 脚手架文案国际化**(硬编码中文框架字随 `video.language` 选中/英两套,非中文输出时消除中英混排,详见 §9.5 已知限制);**跨要点去重判据升级【2026-07-11 验收发现;v0.5 起排在标注化之后】**——16×16 RGB 签名对「同版式不同内容」页(深底 deck/白底文档/黑板/会议网格)分辨力不足,验收与批量试产的误伤率 5/14、9/10、3/5、1/8,现靠 SKILL.md 宿主目验仲裁兜底;候选判据:更细签名网格、边缘密度差、OCR 文本哈希(实测数据见 `docs/test-videos.md` 批量试产结论)
