...
> **运行前奏(每个宿主开工前先解析一次,跨平台可移植的关键)**:
> 1. **解释器** `PYBIN="$(command -v python3 || command -v python)"`——Debian/Ubuntu 与 OpenClaw 网关最小 PATH 下**没有裸 `python`、只有 `python3`**,不解析会第一条命令就崩。
> 2. **skill 目录** `SKILL_DIR=` 本 `SKILL.md` 所在目录的**绝对路径**——各宿主 cwd 常不等于 skill 根(OpenClaw exec cwd=workspace、Codex cwd=项目目录),相对 `scripts/` 会失配。**OpenClaw** 用 `{baseDir}` 令牌;其余平台按 skill 已知位置填绝对路径。
> 3. 下文命令均简写作 `python scripts/xxx.py`,**一律按 `$PYBIN "$SKILL_DIR/scripts/xxx.py"` 执行**。
> **本 repo 内开发/验收**:脚本跑在 `uv` venv 里,把上式换成 `uv run python scripts/xxx.py`。
> **路径约定**:`<OUT>`/`<W>` 全程绝对路径。脚本把 `--work` 原样拼进 `proxy_path`/`final_path`/`candidates.json` 等制品;`storyboard.py validate` 用 `Path(proxy_path).exists()` 校验存在性——中途换相对路径或换 cwd 会误报失败。
>
> **各平台装载速查**(四家共读 agentskills.io 的 `SKILL.md`+YAML frontmatter,按 `description` 触发;frontmatter 只保 `name`/`description` 公共子集,平台专属门控放各自 `metadata.<平台>`):
> - **Claude Code/Desktop**:skill 目录直放。 **Codex**:拖入 `~/.agents/skills/<名>/`(或 repo `.agents/skills`),按 description 隐式触发。
> - **OpenClaw**:放 `~/.agents/skills/<名>/`,**`SKILL.md` 必须实体拷贝**(软链逃逸守卫会拒),`scripts/`/`schemas/`/`assets/` 可软链。 **Hermes**:`~/.hermes/skills/<类>/<名>/`,slash 命令 + description 触发。
...
**node/deno**:setup 若印「未检出 node 或 deno」只是提示、不阻塞——仅 YouTube 取流需其一解 JS 挑战(EJS),本地文件/B 站不需要;缺了再装不迟。

**0b. 宿主视觉能力自测**(决定后续走视觉路还是时间轴路,守零打断放在分析前):`Read` `<SKILL_DIR>/assets/vision_probe.png`,把你在图上看到的**确切文字**交给 checker 比对:
`python scripts/vision_probe.py --claim "<你看到的确切文字>"`。
- exit 0 → **视觉模式**:后续步骤 3/5 照常 Read sheet 目验。
- exit 1(答不出 / 拒读 / 看不见)→ **无视觉模式**:后续按各步骤的「无视觉」分支走时间轴路——核心索引与笔记照常产出,仅幻灯片降级(见步骤 6/8)。**纯文本宿主别编造图上文字**:checker 只认归一化后哈希,编造必判否,如实走无视觉路才是对的。
- **续跑友好**:`<W>` 存在后(步骤 1 之后)带 `--work <W>` 重跑一次,判决落 `.work/host_caps.json`;续跑/新会话开工时先读它恢复模式(存在即不必重读探针图,省 1 次图 Read)。
- 用户可显式声明宿主视觉能力,覆盖本判决。

### 1. 取流
`python scripts/fetch.py --url <URL> --work <OUT>/.work`(B 站加 `--cookies-from-browser chrome`;**headless / 云沙箱无本机 Chrome** 时改用 `--cookies <cookies.txt>` 传导出的 cookies 文件)。
...
> **无视觉模式**:跳过探针 sheet 的 Read/目验(无法目验定性)——**保守处理**:`visual_form` 标 `unknown`(不猜 slide-driven)、**不启用页边界 snap**;轴 B 体裁仍照常从 transcript+标题+分区先验(纯文本)判。核心索引/笔记不依赖轴 A,幻灯片版面权重在无视觉降级下 moot(步骤 8)。以下**目验相关条目仅视觉模式适用**。
...
> **无视觉模式**:**跳过 Read sheet**。为每个叶子直接读 `.work/candidates.json`、按 `node_id` 过滤,**按 `peak_score` 降序取第 1 个**(换页最强的峰后落定帧;**不**按 composite `score`——含密度会挑到工具栏/水印虚高帧),无 peak 的 midpoint 候选退窗口位置。照下方写回格式把 `proxy_path`/`t`/`reason`/`score` 写进 media,并加一句约定标记 `"note": "auto-timeline-unverified"`(「自动按时间轴选取、未经视觉精修」)。dedup 改跑 `--no-vision`(见下),**跳过「去重仲裁」条**(无目验可仲裁)。以下目验相关条目仅视觉模式适用。
...
- 跑 `python scripts/storyboard.py dedup --work <W>`(跨要点去重**标注**:重复组写 `dedup_group`、组内最高分标 `dedup_primary: true`,单帧组为 null;**不删除、不替换任何 media**——唯一性是 slide 视图层策略,见步骤 7)。**无视觉模式加 `--no-vision`**:抬合并门→宁欠勿并,只并近乎同帧,宁可多张可见重复页也绝不静默吞掉不同讲义页(spec §视觉能力处理 6a)
...
用 AskUserQuestion 问一次(此时 video_index.json 已落盘,本问只决定是否继续渲染)。**宿主无 AskUserQuestion 等结构化提问工具时**(Codex/OpenClaw/Hermes),退化为纯文本问同样几项、给同样默认值:
- 「产出形态」slides / markdown 笔记 / 两者 / 仅索引文档(默认 slides)——选「仅索引文档」则按步骤 5.5 的交付说明告知文档落点,到此收束
  - **无视觉模式**:**默认翻成「笔记」**;slides / 两者 列为「需视觉宿主、默认不可用」并附因由。用户仍**坚持要 slides** 时(5b),带风险提示后按步骤 8 的无视觉分支一站式照渲(不赶用户去别处)
...
- `python scripts/frames.py --finalize --work <W> [--cookies-from-browser chrome | --cookies <cookies.txt>]`(B 站高清定稿同样需 cookie;headless 无 Chrome 用 `--cookies <file>`)
...
- **无视觉模式的幻灯片(5b,仅用户在步骤 6 显式确认后渲染)**:①deck 每页加**可见标记**「自动选帧,未经视觉校对」(页脚或角标)——deck 会被转发,让下游看的人也知情,把「静默的错」变「看得见的错」;②交付前言带风险提示(帧未精修 / 可能一页多帧误并 / 挑得非最优);③帧已按 `peak_score` 时间轴选取、去重已 `--no-vision` 宁欠勿并,无需再目验。**前置依赖**:slides 需 `frontend-slides` skill 在场(缺则只能出笔记/索引);其定位同各平台 skill 装载路径。
